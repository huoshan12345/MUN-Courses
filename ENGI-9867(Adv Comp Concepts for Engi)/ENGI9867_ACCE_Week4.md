...
**(a)** Heuristically derive the regular expression for the language recognized by the NFA whose state transition diagram is given aside.  
**(b)** Build the equivalent DFA for this NFA.  
**(c)** Produce the Type-3 grammar recognized by the DFA you found in (b).
...
**(a)** What is the type of the grammar according to the Chomsky hierarchy? Why?  
**(b)** Find the language generated by the grammar.  
**(c)** Design another grammar with a more restrictive type that generates the same language you found in (b).  
